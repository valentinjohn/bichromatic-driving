...
fig, axes = plt.subplot_mosaic([["Q2_sum", "Q2_sum_sim", "Q2_sum_leg", "Q2_sum_leg"]],
                               figsize=(6, 4), sharex=True, sharey=True)
axes['Q2_sum_leg'].set_visible(False)
...
c = axes['Q2_sum'].pcolor(
    delta/1e6, fp2/1e9, datfile[start_time].su0, shading='auto', cmap='hot', zorder=0)

axes['Q2_sum'].set_ylabel('$f_{P4}$ [GHz]')
axes['Q2_sum'].set_xlabel(r'$\Delta f_{P2}$ [MHz]')


axes['Q2_sum'].set_xlim(delta.min()/1e6, delta.max()/1e6)
axes['Q2_sum'].set_ylim(fp4_start, fp4_stop)

ax2 = axes['Q2_sum'].twinx()
ax2.set_ylim(fp2_start, fp2_stop)
ax2.yaxis.set_tick_params(labelright=False)

# *****************************************************************************
# Q2 sum simulated lines
...
for [f_res, n_P2, n_P4] in fres_nP2_nP4:
    ...
    if n_P4-m*n_P2 == 0:
        ...
        fp4 = f_res/(-m*n_P2+n_P4) + n_P2 / \
            (-m*n_P2+n_P4) * (-m*fq-fp2_delta*1e-3)
