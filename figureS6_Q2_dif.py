...
fig, axes = plt.subplot_mosaic([["Q2_dif", "Q2_dif_sim", "Q2_dif_leg", "Q2_dif_leg"]],
                               figsize=(6, 4), sharex=True, sharey=True)
axes['Q2_dif_leg'].set_visible(False)
...
c = axes['Q2_dif'].pcolor(
    delta/1e6, fp2/1e9, datfile[start_time].su0, shading='auto', cmap='hot', zorder=0)

axes['Q2_dif'].set_ylabel('$f_{P4}$ [GHz]')
axes['Q2_dif'].set_xlabel(r'$\Delta f_{P2}$ [MHz]')


axes['Q2_dif'].set_xlim(delta.min()/1e6, delta.max()/1e6)
axes['Q2_dif'].set_ylim(fp4_start, fp4_stop)

ax2 = axes['Q2_dif'].twinx()
